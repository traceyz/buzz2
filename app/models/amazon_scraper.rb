  ...
  def self.get_reviews
    ...
    forum.product_links.each do |product_link|
      ...
      product_link.link_urls.each do |link_url|
        ...
        reviews_from_page(doc,link_url)
        begin
...
def self.reviews_from_page(doc, link_url)
    # forum = Forum.find_by_name("Amazon")
    ...
    doc.css("table#productReviews tr td >  div").each do |review|
      ...
      args = Scraper.unescape(args)
      klass = Object.const_get('AmazonReview')
      amazon_review = klass.new(args)
      unless amazon_review.valid?
        puts "INVALID"
        puts amazon_review.errors.inspect
        puts args.inspect
        next
      end
      amazon_review.save!
      reviews << amazon_review
    end
    reviews
  end
